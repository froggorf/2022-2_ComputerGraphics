...
//enum CameraNum { PRINT = 0, MAIN = 1, UP = 2, FRONT = 3 };
//
////큐브 관련 변수
...
//glm::vec3 cameraDirection[4];
//glm::vec3 cameraUp[4];
//GLboolean flag_perspective_projection = true;
//GLboolean flag_pers = true;
...
//GLuint frameVAO, frameVBO[3];
//GLvoid DrawFrame(GLfloat w_left, GLfloat w_bottom, GLfloat w_right, GLfloat w_top);
////축 관련 함수 와 변수
//GLvoid DrawAxes();
//
//
...
//	DrawFrame(50,50,850,650);
//	DrawFrame(900, 33, 400 + 900, 300 + 33);
//	DrawFrame(900, 366, 400 + 900, 300 + 366);
//	
//	{//MAIN
//		if(flag_pers)
//			flag_perspective_projection = true;
//		else
//			flag_perspective_projection = false;
//		glViewport(50, 50, 800, 600);
//		for (int i = 0; i < 3; ++i) {
//			cameraPos[PRINT][i] = cameraPos[MAIN][i];
//			cameraDirection[PRINT][i] = cameraDirection[MAIN][i];
//			cameraUp[PRINT][i] = cameraUp[MAIN][i];
//		}
//
//		if (draw_ground)
//			ground.Draw();
...
//	{//UP
//		flag_perspective_projection = false;
//		glViewport(900, 366, 400, 300);
//
//		for (int i = 0; i < 3; ++i) {
//			cameraPos[PRINT][i] = cameraPos[UP][i];
//			cameraDirection[PRINT][i] = cameraDirection[UP][i];
//			cameraUp[PRINT][i] = cameraUp[UP][i];
//		}
//
//		if (draw_ground)
//			ground.Draw();
...
//
//	{//FRONT
//		glViewport(900, 33, 400, 300);
//
//		for (int i = 0; i < 3; ++i) {
//			cameraPos[PRINT][i] = cameraPos[FRONT][i];
//			cameraDirection[PRINT][i] = cameraDirection[FRONT][i];
//			cameraUp[PRINT][i] = cameraUp[FRONT][i];
//		}
//		flag_perspective_projection = false;
//		if (draw_ground)
//			ground.Draw();
...
//		break;
//	case 'a':	//카메라 왼쪽 이동
//		cameraPos[MAIN].x -= WorldSize / 100;
//		cameraPos[UP].x -= WorldSize / 100;
//		cameraPos[FRONT].x -= WorldSize / 100;
//		break;
//	case 'd':	//카메라 오른쪽 이동
//		cameraPos[MAIN].x += WorldSize / 100; 
//		cameraPos[UP].x += WorldSize / 100;
...
//		cameraPos[UP].y += WorldSize / 100;
//		cameraPos[FRONT].y += WorldSize / 100;
//		break;
//	case 's':	//카메라 아래쪽 이동
//		cameraPos[MAIN].y -= WorldSize / 100;
//		cameraPos[UP].y -= WorldSize / 100;
...
//		glm::vec4 cd = { cameraDirection[MAIN],1.0 };
//		cd = cd * r;
//		for (int i = 0; i < 3; ++i) {
//			cameraDirection[MAIN][i] = cd[i];
//		}
//	}
//	{//UP
//		glm::mat4 r = glm::mat4(1.0f);
//		r = glm::rotate(r, glm::radians(5.0f), glm::vec3(0.0, 1.0, 0.0));
//		glm::vec4 cu = { cameraUp[UP],1.0 };
//		cu = cu * r;
//		for (int i = 0; i < 3; ++i) {
//			cameraUp[UP][i] = cu[i];
//		}
...
//		glm::mat4 r = glm::mat4(1.0f);
//		r = glm::rotate(r, glm::radians(5.0f), glm::vec3(0.0, 1.0, 0.0));
//		glm::vec4 cd = { cameraDirection[FRONT],1.0 };
//		cd = cd * r;
//		for (int i = 0; i < 3; ++i) {
//			cameraDirection[FRONT][i] = cd[i];
//		}
//	}
//		break;
//	case 'y':
...
//	case 'x':
//	{
//		std::cout << cameraDirection[MAIN][X] << " " << cameraDirection[MAIN][Y]  << " " << cameraDirection[MAIN][Z] << std::endl;
//		glm::mat4 r = glm::mat4(1.0f);
//		r = glm::rotate(r, glm::radians(2.0f), glm::vec3(1.0, 0.0, 0.0));
//		glm::vec4 cd = { cameraDirection[MAIN],1.0 };
//		cd = cd * r;
//		for (int i = 0; i < 3; ++i) {
//			cameraDirection[MAIN][i] = cd[i];
...
//		glm::mat4 r = glm::mat4(1.0f);
//		r = glm::rotate(r, glm::radians(5.0f), glm::vec3(0.0, 1.0, 0.0));
//		glm::vec4 cp = { cameraPos[MAIN],1.0 };
//		glm::vec4 cd = { cameraDirection[MAIN],1.0 };
//		cp = cp * r;
//		cd = cd * r;
//		for (int i = 0; i < 3; ++i) {
//			cameraPos[MAIN][i] = cp[i];
//			cameraDirection[MAIN][i] = cd[i];
//		}
//	}
//	{	//UP
//		glm::mat4 r = glm::mat4(1.0f);
//		r = glm::rotate(r, glm::radians(5.0f), glm::vec3(0.0, 1.0, 0.0));
//		glm::vec4 cp = { cameraPos[UP],1.0 };
//		glm::vec4 cd = { cameraDirection[UP],1.0 };
//		glm::vec4 cu = { cameraUp[UP],1.0 };
//		cp = cp * r;
//		cd = cd * r;
//		cu = cu * r;
...
//		glm::mat4 r = glm::mat4(1.0f);
//		r = glm::rotate(r, glm::radians(5.0f), glm::vec3(0.0, 1.0, 0.0));
//		glm::vec4 cp = { cameraPos[FRONT],1.0 };
//		glm::vec4 cd = { cameraDirection[FRONT],1.0 };
//		glm::vec4 cu = { cameraUp[FRONT],1.0 };
//		cp = cp * r;
//		cd = cd * r;
//		cu = cu * r;
//		for (int i = 0; i < 3; ++i) {
//			cameraPos[FRONT][i] = cp[i];
//			cameraDirection[FRONT][i] = cd[i];
//			cameraUp[FRONT][i] = cu[i];
//		}
//	}
//		
//		break;
...
//		switch (button) {
//		case 3:			//마우스휠 위로 역할
//			cameraPos[MAIN].z -= WorldSize / 100;	
//			cameraPos[UP].z -= WorldSize / 100;
//			cameraPos[FRONT].z -= WorldSize / 100;
//			
//			break;
//		case 4:			//마우스휠 아래로 역할
//			cameraPos[MAIN].z += WorldSize / 100;	//카메라 바깥쪽으로
//			cameraPos[UP].z += WorldSize / 100;
//			cameraPos[FRONT].z += WorldSize / 100;
//			
//			break;
...
//	}
//	{
//		cameraPos[MAIN] = glm::vec3(0.0f, 70.0f, 100);
...
//		}
//		
//		cameraPos[UP] = glm::vec3(0.0f, 100.0f, 0.0);
//		cameraDirection[UP] = glm::vec3(0.0f, -1.0f, 0.0f);
//		cameraUp[UP] = glm::vec3(0.0f, 0.0f, -1.0f);
//
//		cameraPos[FRONT] = glm::vec3(0.0f, 0.0f, 100);
//		cameraDirection[FRONT] = glm::vec3(0.0f, 0.0f, -1.0f);
//		cameraUp[FRONT] = glm::vec3(0.0f, 1.0f, 0.0f);
//
//
//		flag_perspective_projection = true;
//		flag_pers = true;
...
//			std::cout << "ERROR::SHADER_                                                                 COMPILATION_ERROR of type: " << type << "\n" << infoLog << "\n -- --------------------------------------------------- -- " << std::endl;
//		}
//	}
//	else
//	{
...
//	GLfloat left = (w_left - w) / w;
//	GLfloat right = (w_right - w) / w;
//	GLfloat top = (w_top - h) / h * -1;
//	GLfloat bottom = (w_bottom - h) / h * -1;
//
//	GLfloat pos[4][3] = {
//		{left,bottom,0.0},
//		{left,top,0.0},
...
//
//	glBindBuffer(GL_ARRAY_BUFFER, frameVBO[COLOR]);
//	glBufferData(GL_ARRAY_BUFFER, sizeof(color), color, GL_STATIC_DRAW);
//	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, 0);
//	glEnableVertexAttribArray(1);
//
//	glBindBuffer(GL_ARRAY_BUFFER, frameVBO[POS]);
//	glBufferData(GL_ARRAY_BUFFER, sizeof(pos), pos, GL_STATIC_DRAW);
//
//	/*glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, frameVBO[INDEX]);
//	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(index), index, GL_STATIC_DRAW);*/
//	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), 0);
//	glEnableVertexAttribArray(0);
//	
...
//	unsigned int viewLocation = glGetUniformLocation(s_program, "view");
//	unsigned int projectLoc = glGetUniformLocation(s_program, "projection"); 
//	glUniformMatrix4fv(modelLocation, 1, GL_FALSE, &temp[0][0]);
//	glUniformMatrix4fv(viewLocation, 1, GL_FALSE, &temp[0][0]);
//	glUniformMatrix4fv(projectLoc, 1, GL_FALSE, &temp[0][0]);
//
//
//	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
